...
- [x] Full ML pipeline in `telco_xgboost.ipynb`
- [x] Trained XGBoost model saved to `churn_model.pkl`
- [x] Working Streamlit application (`app.py`)
...
The Streamlit app (`app.py`) loads `churn_model.pkl` and `model_columns.pkl` to serve real-time predictions. Retention agents and non-technical stakeholders can input a customer profile and instantly receive:
